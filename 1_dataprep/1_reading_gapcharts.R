# Purpose of script: to prep the READING data for the multiple demographic comparisons of the most recent cohort. 
...
# *Race ----
black_students <-
  read_csv("1_dataprep/data/raw/2024_black_students_all_divisions_read.csv") %>%
...
white_students <- 
  read_csv("1_dataprep/data/raw/2024_white_students_all_divisions_read.csv") %>%
  rename_with(.cols = everything(), .fn = ~str_replace_all(str_to_lower(.x),  " ", "_")) %>%
  select(school_year, division_number, division_name, level = race, test_level, pass_count, total_count, pass_rate) %>%
  mutate(
    test_year = as.numeric(str_sub(school_year, 6,9)),
...
# *Ethnicity ----
hispanic_students <- 
  read_csv("1_dataprep/data/raw/2024_hispanic_students_all_divisions_read.csv") %>%
  rename_with(.cols = everything(), .fn = ~str_replace_all(str_to_lower(.x),  " ", "_")) %>%
  select(school_year, division_number, division_name, level = race, test_level, pass_count, total_count, pass_rate) %>%
  mutate(
    test_year = as.numeric(str_sub(school_year, 6,9)),
...
# *SES Advantage ----
ses <-
  read_csv("1_dataprep/data/raw/2024_by_advantage_all_divisions_read.csv") %>%
  rename_with(.cols = everything(), .fn = ~str_replace_all(str_to_lower(.x),  " ", "_")) %>%
...
# *Overall Rates ----
overall <-
  read_csv("1_dataprep/data/raw/2024_all_students_all_divisions_read.csv") %>%
  rename_with(.cols = everything(), .fn = ~str_replace_all(str_to_lower(.x),  " ", "_")) %>%
...
# Save ----
write_csv(complete_data, "1_dataprep/data/processed/1_complete_reading.csv")
