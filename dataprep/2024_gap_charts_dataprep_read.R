...
# Race
black_students <-
  read_csv("dataprep/data/2005_2024_data/2024_black_students_all_divisions_read.csv") %>%
...
white_students <- 
  read_csv("dataprep/data/2005_2024_data/2024_white_students_all_divisions_read.csv") %>%
  rename_with(.cols = everything(), .fn = ~str_replace_all(str_to_lower(.x),  " ", "_")) %>%
  select(school_year, division_number, division_name, level = race, test_level, pass_count, total_count, pass_rate) %>%
  mutate(
    test_year = as.numeric(str_sub(school_year, 6,9)),
...
# Ethnicity
hispanic_students <- 
  read_csv("dataprep/data/2005_2024_data/2024_hispanic_students_all_divisions_read.csv") %>%
  rename_with(.cols = everything(), .fn = ~str_replace_all(str_to_lower(.x),  " ", "_")) %>%
  select(school_year, division_number, division_name, level = race, test_level, pass_count, total_count, pass_rate) %>%
  mutate(
    test_year = as.numeric(str_sub(school_year, 6,9)),
...
ses <-
  read_csv("dataprep/data/2005_2024_data/2024_by_advantage_all_divisions_read.csv") %>%
  rename_with(.cols = everything(), .fn = ~str_replace_all(str_to_lower(.x),  " ", "_")) %>%
...
# Overall Rates
overall <-
  read_csv("dataprep/data/2005_2024_data/2024_all_students_all_divisions_read.csv") %>%
  rename_with(.cols = everything(), .fn = ~str_replace_all(str_to_lower(.x),  " ", "_")) %>%
...
# Export it ---------------------------------------------------------------
# Complete data
write_csv(complete_data, "dataprep/data/2024_complete_data_read.csv")
