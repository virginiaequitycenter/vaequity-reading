# Explore gaps in MATH pass rates 
...
tornado <- read_csv("3_math_tornado.csv")
...
ui <- page_fillable(
  ...
  # Application title
  titlePanel(h1(strong("SOL Math Gaps by School Division"), align = "center"), 
             "SOL Math Gaps by School Division"),
...
server <- function(input, output) {
  ...
  output$tornadoPlot <- renderPlotly({
    ...
    tornado_plot <- ggplot(rvs$plt_dat, aes(y = division_name, x = rate, group = division_name, color = level,
      ...
      geom_vline(xintercept = c(0.25, 0.5, 0.75), linetype = "dotted", color = "light grey")
