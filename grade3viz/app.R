...
ui <- fluidPage(
    ...
    tags$p("Highlight a selected school division (in red) to view the reading SOL proficiency pass rate relative to all school divisions."),
...
server <- function(input, output) {
    ...
    output$traceplot <- renderPlotly({
      grade3_prof %>% 
        ...
        labs(title = "Reading SOL Percent Proficient, 2013-2021", 
             x = "Year (spring)", y = "% Proficient",
        ...
        geom_line(data = target(), aes(y = pass_proficient_rate, label = division_name), color = "firebrick") +
